...
- [ ] Activer GitHub Pages côté serveur : Settings > Pages > Source: `main` branch + dossier `/docs`. Attendre 1-10 min, vérifier l'URL `https://sachazaouati.github.io/illusion-petits-nombres/` — action manuelle GitHub UI uniquement
- [ ] Initialiser `renv` : lancer `renv::init()` depuis RStudio à la racine (crée `renv/`, `renv.lock`, `.Rprofile` automatiquement — ne pas créer ces fichiers à la main)
